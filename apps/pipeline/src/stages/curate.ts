 ...
 *
 * Applied here rather than in each collection path: both paths feed this
 * merge, and doing it before dedupe means two sources that differ only in
 * encoding ("Tea &#038; Tour" vs "Tea & Tour") now match instead of both
 * being kept.
 */
const TEXT_FIELDS = [
	"title",
...
export interface CurateOptions {
	/** Gates dedupe's ambiguous-pair classifier — missing means "keep both". */
	googleApiKey?: string;
	/** Gates the locality (other-city / other-country) check — missing means
	 * "every location kept unchecked". */
...
export async function curate(
	...
	const COST_LOCALE = {
		...
		// Needed by the schema.org dates on every page: a naive wall-clock
		// string is ambiguous to a crawler. See isoWithOffset.
		timezone: cityCfg.timezone,
	};

	const { monday, sunday } = ctx.week;
	// Same window the scrape pass uses: today through the end of next week.
	const WINDOW_FROM = toISODate(new Date(), CITY_TZ);
	const WINDOW_TO = addDays(
		toISODate(sunday, CITY_TZ),
		ctx.config.publish.windowDaysAfterWeek,
	);
	...
	/**
	 * Skip only when this week's digest exists AND no curated input is newer
	 * than it. The week stamp alone missed a real case: a provider that failed
	 * in the morning run (max_tokens, nothing written) was collected by a
	 * re-dispatch 20 minutes later, and curate skipped on the stamp — 16
	 * events sat on disk, invisible until someone passed FORCE. mtime is
	 * trustworthy here: a checkout stamps every file at the same instant, so
	 * anything written during the run is strictly newer. Rank and geocode
	 * follow on their own, because writeJson drops ranked_at/geocoded_at.
	 *
	 * ponytail: within one run only — a fresh checkout stamps inputs and
	 * output alike, so an input committed by an earlier run is never "newer".
	 * Record the consumed inputs (path + hash) in the digest if cross-run
	 * matters.
	 */
	function alreadyCuratedThisWeek(): boolean {
		...
		for (const f of newer.slice(0, 5)) log.log(`    ${relative(DATA_ROOT, f)}`);
		return false;
	}
	 ...
	 * per-provider yield report is what decides whether a paid provider is
	 * worth keeping on. */
	...
	/**
	 * How much each provider actually contributed: events that survived
	 * dedupe, and events no other provider also found. The second number is
	 * the one that matters for a paid provider — a search that only re-finds
	 * what the scrapers already have is pure cost.
	 */
	function reportProviderYield(
		...
		log.log("→ provider yield (events kept / found by that provider alone):");
	...
	/**
	 * Records which sources the search actually found events on, so next
	 * week's prompts can name only those (see sourceYield.ts). Hosts
	 * belonging to no source are counted too and reported: the search
	 * finding a venue we have never heard of is exactly the signal
	 * probe-sources wants.
	 */
	function updateYieldLedger(week: string, linkHosts: string[]): void {
		...
		if (worth.length > 0) {
			...
			log.log(`    ${worth.map((w) => `${w.host} (${w.count})`).join(", ")}`);
		}
	}
	...
	function cleanEvent(event: Record<string, unknown>): Record<string, unknown> {
		...
		// Re-derived, never trusted as written. The scrape path already builds
		// this with humanDatetime, but the AI search path takes `datetime`
		// straight from the model, which produced 24 different shapes in one
		// city ("7-13 Sept", "12 September 2026", "Tue–Sun, 10am–5pm", "Fri 11
		// Sep, evening"). Every provenance funnels through cleanEvent, so this
		// is the one place that can make them agree. The fallback keeps the
		// model's own string for an event with no parsable ISO at all, rather
		...
		out.datetime =
			...
			"";
		// A foreign currency on a South East Queensland listing is the
		// source's markup being wrong, not a real price. See
		// normaliseCurrency.
		if ("cost" in out)
		...
		// The council's Trumba embed link lands on its event search, not the
		// event.
		if (typeof out.link === "string")
			out.link = councilEventUrl(out.link) ?? "";
		if (Array.isArray(out.tags)) {
			out.tags = out.tags.map((t) => cleanText(t).trim().toLowerCase());
		}
		// Stricter than the other URLs: an extension-less image URL is an
		// extraction fault rather than a real picture, and 61 of 139 were
		// dead. See isLikelyImageUrl.
	...
	/**
	 * Throws out events that are not in the city being published, under two
	 * rules of different strength.
	 *
	 * **Wrong city** — the distance test — applies only to the `aggregators`
	 * and `open` tiers. Those are the tiers where a source promoted "for
	 * Brisbane" turns out to be national: musick.com.au is a country-wide gig
	 * guide, and its verified listing page put 40 Sydney, Melbourne, Adelaide
	 * and Perth events into one Brisbane week. An `institutions` or
	 * `independents` source is a venue's own site listing its own events, so
	 * it is trusted here — and it has to be, because "The Princess Theatre"
	 * geocodes to Melbourne while being a real Brisbane venue.
	 *
	 * **Wrong country** applies to every tier. That is never a near-miss or
	 * an ambiguous venue name, and the tier trust turned out to be
	 * exploitable: a `method: scraper` source verified for Brisbane
	 * (creativelunchclub.com) lists meetups worldwide, and put eight events
	 * in Vienna, Manchester, Berlin, Stockholm, Portland, Montreal, Zurich
	 * and London on the site under an `independents` entry — where nothing
	 * ever looked at them.
	 *
	 * Only the distinct location strings are geocoded, never one call per
	 * event, and every answer is cached to disk, so widening the subject set
	 * to all tiers costs one run's worth of new venues and nothing
	 * thereafter.
	 */
	async function dropOtherCities(
		...
		// Same shape of degradation as the dedupe classifier below: the check
		// is an improvement on top of curation, never a precondition for it,
		// and dropping is the destructive direction — so no key means keep
		// everything.
		if (!apiKey) {
			log.log("  ⚠ GOOGLE_MAPS_API_KEY unset — every location kept unchecked");
			return events;
		}
		if (!centre) {
			log.log(
		...
		// remedies — a wrong-city hit means an aggregator needs watching, a
		// foreign one means the source's listing page is not what it was
		// promoted as.
		for (const [location, why] of elsewhere)
			log.log(`    ✗ ${location} — ${why}`);
		...
		return events.filter((e) => {
			...
			// Every tier, not just aggregators. The old rule assumed a
			// venue-tier source only lists its own city, and 128 scraper
			// sources later that is plainly false: QTIX ticketed a Toowoomba
			// museum workshop under brisbane's institutions, and a
			// life-drawing organiser listed Miami Marketta on the Gold Coast
			// under independents. Both were already being geocoded and
			// correctly identified as elsewhere — the drop just never applied
			// to them.
			return !elsewhere.has(location);
		});
	}

	async function mergeAndDeduplicate(): Promise<Record<string, unknown>[]> {
		...
		// sources the *search* prompts name, and a scraped source is not in
		// that list at all.
			...
			try {
				...
				if (payload.week_start === toISODate(monday, CITY_TZ)) {
					...
					for (const rawEvent of events) {
						...
						// The scrape path windows its own output, but the AI
						// search path never did, so finished and far-future
						// events reached the site from search only. Apply the
						// one rule here so both paths are governed
						// identically.
						...
						if (start) {
							...
							// Undated events are kept: they cannot be shown to
							// be past, and the site still renders their human
							// date string. Dropping them would lose real
							// events on the word of a missing field.
							dropped.undated++;
						}
		...
		// Carry the previous digest forward. The run is on Sunday for the
		// week starting Monday, and the per-source input files above were
		// already overwritten with next week's results by collect — so
		// without this, today's remaining events (and anything found last
		// week that is still upcoming) would vanish from the site the
		// morning they are being planned around. Appended after the fresh
		// events so dedupe's tie-break keeps the fresh record. Undated
		// events are not carried: they can never expire via isPast, so they
		// would accumulate forever. Everything else does expire on the
		// following run, so this cannot grow unbounded.
		const carried = {
			kept: 0,
			total: 0,
			past: 0,
		...
		for (const rawEvent of previousEvents()) {
			...
			// Stripped before dedupe on purpose: with the filler gone,
			// dedupe's completeness tie-break prefers a freshly scraped copy
			// that has a real description over the carried one.
		...
		// After the merge so both collection paths are governed identically,
		// and after the window so a finished event is never paid for.
		...
		// Deliberately not the old dedupeEvents here: this merge now spans
		// many per-source scrape files plus the search results, so it gets
		// the blocking + grey-zone strategy in dedupe.ts. See that file's
		// header for why and what it costs.
		// The classifier is an enhancement, not a requirement: without it
		// the deterministic stage still runs and ambiguous pairs are simply
		// kept. Merging is the destructive direction, so degrading to "keep
		// both" is the safe failure — and it means a missing key can't take
		// down curation.
		...
		// Collapse tag variants now the whole week is in one place — this is
		// the only point that can see the corpus, and the merge is
		// corpus-driven on purpose (see mergeTagVariants).
		...
		for (const event of events) {
			...
			// "free" is a fact about the cost field, not a judgement, so
			// code sets it rather than the annotator — which never sees
			// cost. Asked to guess, it agreed with the cost on only 17 of 43
			// free events, erring in both directions.
			const cost = ((event.cost as string) ?? "").trim();
			const isFree = /^(free|free entry|no charge|\$?0(\.00)?)$/i.test(cost);
			// "standup" without "comedy" hides stand-up from the comedy
			// filter — see TAG_MATCHERS.
	...
	function writeJson(events: Record<string, unknown>[]): string {
		const payload = {
			city: CITY_NAME,
			city_key: CITY,
	...
	if (!FORCE && alreadyCuratedThisWeek()) {
		log.log(
			"→ Already curated for this week — skipping. Set FORCE=true to re-curate.",
		);
		return { skipped: true, eventCount: 0, outPath: OUT_PATH };
	}
	...
	log.log("→ Merging and deduplicating…");
	...
	log.log(`→ ${events.length} events total`);
	writeJson(events);
	log.log("✓ Curation complete.");
	return { skipped: false, eventCount: events.length, outPath: OUT_PATH };
}
