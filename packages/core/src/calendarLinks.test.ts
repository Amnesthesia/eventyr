...
test("a timed event keeps its wall-clock time, unconverted", () => {
	const url = googleCalendarUrl(ev({}), "brisbane", BNE) ?? "";
...
test("Outlook carries the offset inline, since it has no ctz", () => {
	const url = outlookCalendarUrl(ev({}), "brisbane", BNE) ?? "";
...
test("an event with no usable date offers no calendar route at all", () => {
	...
	assert.equal(googleCalendarUrl(undated, "brisbane", BNE), null);
	assert.equal(outlookCalendarUrl(undated, "brisbane", BNE), null);
	...
	assert.deepEqual(
		calendarLinks(undated, "brisbane", BNE).map((l) => l.key),
		["download"],
	);
});
...
test("every route is offered for a normal event", () => {
	assert.deepEqual(
		calendarLinks(ev({}), "brisbane", BNE).map((l) => l.key),
