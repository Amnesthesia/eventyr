...
| D12 | City time zones | **`timezone:` (IANA) in `sources/{city}.yml` is the only place a city's zone is stated**, per your answer. PR 0 removes every hard-coded zone and offset: `BRISBANE_UTC_OFFSET_HOURS`, the `+10:00` literals, `?? "Australia/Brisbane"` fallbacks and default parameters, and the `TZ=Australia/Brisbane` pins in `digest.yml`, `deploy.yml` and `reprobe.yml`. It also makes date parsing and "today" and week boundaries DST-aware and independent of the machine's time zone. Byron becomes `Australia/Sydney`. There's no separate `timezone_offset` field, because offsets are derived per date from `timezone`. | **PR 0** |
...
| D19 | **Speed and cost are invariants** (your instruction) | Everything that runs concurrently today stays concurrent: provider searches (`Promise.all`, `collection.ts:160`), batched prompts under the per-provider limiter (`mapWithConcurrency`), and scrape fan-out. **No refactor may introduce a serial loop over independent work.** The per-stage usage-and-cost report printed at exit (`installUsageReporting`) is kept byte-for-byte. It becomes `llm`'s `usageTotals()`, printed by the pipeline's `io/usage.ts`, and it also feeds the manifest. Provider batching (D14) adds cost savings on top of concurrency and doesn't replace it: batch jobs from different stages and cities run concurrently, and the cancel-and-sync fallback runs its leftovers concurrently. These are enforced by the concurrency-profile and cost-report checks in §9. | 1.6, 1.7, 1.8, 1.11 |
...
| Sub | File | Summary |
|---|---|---|
| 0.1 | `phase-0.01-byron-timezone.md` | `timezone:` in the city YAML becomes the only source of a city's zone. Every hard-coded zone and offset is removed, including the workflow `TZ` pins, and the output is proven identical under `TZ=UTC`, `TZ=Australia/Brisbane` and `TZ=America/Los_Angeles`. Byron becomes `Australia/Sydney` and joins `weekly.yml`. `currency` is written explicitly for every city. The Queensland cities stay byte-identical. |
...
| **Concurrency** | The replay harness serves canned responses with a fixed artificial latency (for example 200 ms), recording **peak in-flight requests per provider** and **wall-clock per CLI** on the old code. After each migration, peak in-flight must be ≥ the old value (equal, with the same ceilings), and wall-clock must be within +10%. A drop means something went serial. | 1.6, 1.7, 1.8, 1.11, 1.13 |
| **Cost report** | The usage/cost report each CLI prints at exit is captured in replay mode, where usage comes from the canned responses so it's deterministic, and must match byte for byte | 1.6, 1.7, 1.11, 1.13 |
...
| R22 | **A refactor quietly serialises work** (for example `for … await ask()` replacing `mapWithConcurrency`), which is slower with the same output | Concurrency profile check (§9). `ask(string[])` is the default way to run independent prompts. The CLAUDE.md rule "never a serial loop over independent work" is checked in review |
...
None. Everything is decided (§0). PR 0 is next, and you're running it yourself.
