...
export default function FilterBar({
  categories, activeCat, dateRange, activeTags,
  weekStart, weekEnd,
  onCatChange, onDateChange, onTagRemove,
}: Props) {
      ...
      <div className="filter-bar">
        ...
        <span className="filters">
          <button
            ...
            onClick={() => onDateChange(isToday ? null : { start: today, end: today })}
          >
            Today
          </button>
          <button
            className={`filter-btn date-filter${isTomorrow ? ' active' : ''}`}
            onClick={() => onDateChange(isTomorrow ? null : { start: tomorrow, end: tomorrow })}
          >
            Tomorrow
          </button>
          <DateRangePicker value={pickerValue} onChange={onDateChange} minDate={weekStart} maxDate={weekEnd} />
        </span>
      </div>
      {activeTags.length > 0 && (
