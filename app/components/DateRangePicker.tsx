...
import { parseDate, getLocalTimeZone, today as getToday } from '@internationalized/date'
...
export default function DateRangePicker({ value, onChange, minDate, maxDate }: Props) {
  ...
  return (
    <span className="date-range-wrapper" ref={wrapperRef}>
      ...
      {open && (
        <div className="date-picker-popover" role="dialog" aria-label="Select date range">
          <RangeCalendar
          ...
          >
            ...
            <CalendarGrid className="cal-grid">
              <CalendarGridHeader>
                {day => <CalendarHeaderCell className="cal-header-cell">{day}</CalendarHeaderCell>}
              </CalendarGridHeader>
              <CalendarGridBody>
