...
7. **`src/ai.ts`** (`pnpm build-ai`) — generates `apps/web/public/ai/{city}/{date|week-date}.json` +
...
- **`src/ai.ts`** generates `apps/web/public/ai/index.json` (every city's slug/timezone/data_as_of and the
